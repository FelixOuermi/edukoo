...
- Éléments identifiés comme manquants par rapport à un bulletin "complet" (secondaire francophone), non traités dans cette passe — à arbitrer avant de les construire (ce sont des ajouts de fond, pas des corrections) :
  - Moyenne de la classe par matière (actuellement seulement la moyenne générale de l'élève, pas de comparaison par matière sur le bulletin lui-même — elle existe déjà dans Statistiques mais n'est pas reprise ici).
