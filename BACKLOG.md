...
- Éléments identifiés comme manquants par rapport à un bulletin "complet" (secondaire francophone), non traités dans cette passe — à arbitrer avant de les construire (ce sont des ajouts de fond, pas des corrections) :
  - Décision du conseil de classe (passage/redoublement) — cohérent avec le manque déjà noté ailleurs ("conseil de classe" absent du modèle scolaire).
  - Photo de l'élève — `students.photo_url` existe déjà en base mais rien ne l'alimente ni ne l'affiche nulle part dans l'appli.
