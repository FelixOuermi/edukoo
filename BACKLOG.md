...
### Correction — accès incohérent enseignant/bulletins — demandé le 2026-09-06
- [x] **N'importe quel enseignant pouvait consulter le bulletin complet (classement, toutes matières) de n'importe quelle classe de l'école** alors que la saisie de notes lui reste strictement limitée à ses affectations (`teacher_subjects`). Corrigé aux 4 endroits concernés, avec la même règle qu'ailleurs (classe assignée requise pour un enseignant, aucune restriction pour le directeur) :
  - `dashboard/bulletins/page.tsx` : liste de classes du menu déroulant filtrée aux classes assignées (comme la page Notes) ; une classe demandée par l'URL hors de cette liste est ignorée plutôt que servie.
  - `dashboard/bulletins/actions.ts` (`saveAppreciation`) : vérifie l'affectation à la classe de l'élève avant d'écrire.
  - `dashboard/bulletins/classe/[classId]/pdf` et `dashboard/bulletins/eleve/[studentId]/pdf` : un lien direct forgé (hors du menu) ne suffit plus à contourner la restriction — ces deux routes vérifient elles-mêmes l'affectation et renvoient 403 sinon.
  - **Testé en production** : compte enseignant temporaire créé, affecté à une seule classe/matière (3ème B / Mathématiques) sur l'école réelle. Vérifié que son menu Bulletins n'affiche que cette classe, qu'une URL forgée vers le PDF d'une autre classe (6ème A) renvoie `{"error":"Vous n'êtes pas affecté à cette classe."}`, qu'une URL forgée vers le bulletin d'un élève de cette autre classe est bloquée de la même façon, et que l'accès à sa propre classe fonctionne toujours normalement. Compte de test (utilisateur auth, ligne `teachers`, affectation `teacher_subjects`) entièrement supprimé après le test.
- [x] **Guide d'utilisation** — `src/app/guide` passe de 18 à 23 sections : ajout des 4 fonctionnalités qui manquaient entièrement (cycles et niveaux, élèves à surveiller, synthèses par IA, Personnel et paie) et d'une section dédiée au fonctionnement hors connexion (PWA lecture seule + saisie notes/absences en attente, jamais mentionnés). Titre "Notes et bulletins" mis à jour pour refléter le découpage du menu en deux pages, section Notes complétée avec l'appréciation par matière et la validation 0-20, section Statistiques complétée avec la mention "Élèves à surveiller" et le bouton de synthèse IA, formulation du support alignée sur celle déjà corrigée dans la FAQ ("fait de son mieux pour répondre rapidement" plutôt qu'un délai non tenu). Vérifié dans le navigateur (sommaire et sections vérifiés après renumérotation).
- [x] **Site vitrine (FAQ)** : déjà corrigé dans une passe précédente de cette même session — plus de claim de clients confirmés en Côte d'Ivoire/Sénégal/Mali (reformulé en "fonctionne tout aussi bien... mais notre développement reste concentré sur le Burkina Faso") et le délai de support non tenable a été retiré. La liste d'avantages sur l'accueil (révision P2 de début septembre) a depuis été complétée avec le hors-ligne et la détection à risque (voir plus haut), mais ne reflète toujours pas l'IA ni Personnel & Paie — pas traité dans cette passe.

### Audit contenu des bulletins — demandé le 2026-09-06
- [x] **Rang sans dénominateur** — un bulletin affichait "8e" sans dire sur combien d'élèves, peu lisible isolément. `StudentBulletin.rankedOutOf` ajouté (nombre d'élèves classés, moyenne non nulle), affiché "8/10" sur `dashboard/bulletins` et "8e / 10" sur le PDF.
- [ ] **Bulletin liste TOUTES les matières de l'école, pas seulement celles de la classe** — `subjects` vient d'une requête globale par école, sans filtrer par ce que la classe étudie réellement ; pour une école avec des matières différentes par cycle (ex. Philosophie seulement en Terminale), chaque bulletin afficherait des matières hors-programme avec un tiret. **Tentative de correction puis annulée** : `teacher_subjects` semblait la source d'autorité logique (affectation enseignant↔classe↔matière), mais vérifié en production que cette table est **vide** pour de vraies classes ayant pourtant des notes (le directeur note directement sans jamais passer par l'affectation, qui n'est obligatoire que pour un compte enseignant non-directeur — `assertTeacherAssigned` dans `notes/actions.ts`). Un filtre de repli "matière = a au moins une note" aurait alors caché silencieusement toute matière pas encore notée ce trimestre — pire que le problème d'origine (un directeur ne verrait plus qu'un enseignant a oublié de noter, au lieu de voir un tiret qui l'alerte). Reste ouvert, lié à l'extension cycle/niveau déjà notée en P3 comme "reste à faire".
- [x] **Appréciation par matière et par enseignant** — `bulletin_appreciations` gagne une colonne `subject_id` nullable (`supabase/migrations/0026_subject_appreciations.sql`) : NULL = appréciation générale (comportement existant, inchangé), renseignée = appréciation d'une matière précise. Contrainte d'unicité remplacée par deux index uniques **partiels** (au plus une ligne générale et une ligne par matière et par élève/année/trimestre) plutôt qu'une seule contrainte à 3 colonnes, incompatible avec plusieurs appréciations par matière pour le même élève. Saisie directement sur la page Notes (nouvelle colonne "Appréciation (matière)", à côté des notes, par l'enseignant de la matière déjà affiché à cet endroit) ; affichée en italique sous la ligne de la matière concernée sur le bulletin PDF. **Piège découvert en testant en direct** : un index unique partiel n'est pas inférable par `ON CONFLICT (colonnes)` sans y répéter sa clause `WHERE`, ce que l'upsert de `supabase-js` ne permet pas d'exprimer — la première version plantait avec *"there is no unique or exclusion constraint matching the ON CONFLICT specification"*, y compris sur l'appréciation générale déjà existante (régression introduite par ce chantier). Corrigé dans les deux actions concernées (`dashboard/notes/actions.ts` et `dashboard/bulletins/actions.ts`) en remplaçant l'upsert par une lecture puis un update/insert explicite. Testé en production de bout en bout après correctif : appréciation saisie sur un élève réel, vérifiée en base, vérifiée sur le bulletin PDF généré (apparaît bien sous "Anglais"), puis supprimée.
- [x] **Moyenne de classe par matière sur le bulletin** — `SubjectGrade.classAverage` ajouté (`src/lib/bulletin.ts`) : moyenne simple des notes des élèves de la classe ayant une note dans cette matière, même méthode que `src/lib/statistics.ts` (`subjectStats`) pour rester cohérent avec la page Statistiques. Nouvelle colonne "Moy. classe" sur le bulletin PDF, entre la note de l'élève et sa moyenne pondérée. **Vérifié en production** en recalculant indépendamment la moyenne des 6 matières de 3ème B à partir des notes brutes (requête directe) : les 6 valeurs recalculées correspondent exactement à celles affichées sur le PDF — la coïncidence apparente (13.17 identique sur toutes les matières) est une propriété des données de démonstration (chaque élève y a le même multi-ensemble de notes réparti différemment par matière), pas un bug.
- [x] **Décision du conseil de classe (passage/redoublement)** — nouvelle table `class_council_decisions` (`supabase/migrations/0027_class_council_decisions.sql`) : une ligne par élève et par **année scolaire** (pas par trimestre — c'est une décision annuelle, généralement prise après le 3e trimestre, contrairement à l'appréciation qui existe par trimestre). Décision sensible : écriture réservée au directeur **au niveau RLS** (même principe que `staff_members`/`payroll_payments`), lecture ouverte à tout le personnel de l'école ainsi qu'au parent et à l'élève concernés (comme `bulletin_appreciations`). Saisie via une nouvelle colonne "Décision (année)" sur `dashboard/bulletins` (menu déroulant + commentaire libre, `class-council-cell.tsx`) — affichée en lecture seule pour un enseignant non-directeur (le composant masque le formulaire plutôt que de le désactiver). Affichée sur le bulletin PDF dans un encart dédié, uniquement quand une décision a été prise (rien avant, pour ne pas laisser croire à une décision "par défaut" sur un bulletin intermédiaire). Testé en production de bout en bout : décision et commentaire saisis sur un élève réel, vérifiés en base, vérifiés sur le bulletin PDF généré, puis supprimés. La restriction "directeur uniquement" réutilise exactement le pattern déjà vérifié pour Personnel & Paie plus haut dans ce fichier (contrôle applicatif + policy RLS indépendante) — non re-testée séparément avec un compte enseignant cette fois pour éviter de perdre à nouveau la session directeur en cours de test (déjà arrivé une fois dans cette session).
- Éléments identifiés comme manquants par rapport à un bulletin "complet" (secondaire francophone), non traités dans cette passe :
  - Photo de l'élève — `students.photo_url` existe déjà en base mais rien ne l'alimente ni ne l'affiche nulle part dans l'appli. **Écarté délibérément** (2026-09-06) : sur un bulletin déjà identifié par nom/matricule/classe, le gain est marginal (utile surtout pour distinguer des bulletins physiques imprimés en masse) alors que le coût (bucket de stockage, formulaire d'upload, validation, gestion de l'ancien fichier, intégration PDF) ne l'est pas — pas un bon rapport effort/valeur pour l'instant.

### Page de modification d'un élève — demandé le 2026-09-06
- [x] **Aucune page pour modifier un élève après sa création** — `updateStudent` existait dans `src/app/dashboard/eleves/actions.ts` mais n'était appelé par aucun composant (code mort). Nouvelle route `dashboard/eleves/[id]/modifier` (`page.tsx` + `edit-student-form.tsx`, formulaire pré-rempli sur le modèle du formulaire de création, avec en plus le statut actif/suspendu/parti). `updateStudent` réécrit au format `useActionState` standard (`_prevState, formData`), valide l'existence de l'élève et la présence du nom, journalise un changement de classe ou de statut dans le journal d'audit (`"Modification élève"`, avec l'ancien et le nouveau libellé), puis redirige vers la fiche élève. Bouton "Modifier" ajouté sur `dashboard/eleves/[id]` (visible à tout le personnel, comme `updateStudent` l'autorise déjà). Corrigé au passage : le statut de l'élève s'affichait en anglais brut sur la fiche (`Suspended` au lieu de `Suspendu`) — même table de traduction que la liste des élèves appliquée ici.
- Testé en production sur un élève réel (Seydou Bationo) : formulaire correctement pré-rempli, changement de statut actif→suspendu→actif confirmé en base et dans le journal d'audit à chaque étape, puis élève remis dans son état d'origine et les deux entrées de test supprimées du journal.
- `deleteStudent` existe aussi dans `actions.ts` mais reste du code mort (aucun bouton de suppression) — trouvé en passant, pas demandé, pas traité.
