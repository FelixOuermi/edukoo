...
import { postParentMessage } from './message-actions'
...
export function MessageThread({ studentId, messages }: { studentId: string; messages: Message[] }) {
  const [state, formAction, pending] = useActionState(postParentMessage, initialState)

  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
      <h2 className="font-semibold text-gray-900">{t.messagesWithSchool}</h2>

      {messages.length === 0 ? (
        <p className="text-sm text-gray-400">{s.noMessages}</p>
      ) : (
          ...
          {messages.map((m) => (
            <li
              ...
              className={`rounded-lg px-3 py-2 text-sm max-w-[85%] ${
                m.sender_role === 'parent' ? 'bg-violet-50 ml-auto text-right' : 'bg-gray-50'
              }`}
            >
      ...
      <form action={formAction} className="flex gap-2 pt-3 border-t border-gray-100">
        ...
        <input
          type="text"
          name="body"
          placeholder={s.writeMessagePlaceholder}
          required
